import OBSWebSocket, {OBSWebSocketError, OBSRequestTypes} from "obs-websocket-js";
...
const ObsController = () => {
    ...
    const sceneTransitions: Array<SceneTransition> = JSON.parse(localStorage.getItem('obs-scene-transitions') || '[]');
    ...
    const messageEventListener = async (event: MessageEvent<string>) => {
        ...
        if (transition !== undefined && transition.UIEvent !== currentUI) {
            console.log(`UI changed from ${currentUI} to ${transition.UIEvent} - changing scene to ${transition.SceneName}`);
    ...
    const openObsConnection = async () => {
        ...
        try {
            ...
            setErrors(errors.concat(`Failed to open OBS WebSocket: ${JSON.stringify(e)}`))
            setObsIsConnecting(false);
        }
    }
    ...
    return <>
        {errors.slice(-10).map(((element, index) => <div key={index}>{element}</div>))}
    </>;
};
